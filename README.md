...
| 报告追问           | 已接入 | ReAct 自主调用报告工具，回答并返回参考时间点      |
...
1. Agent 先根据用户问题决定调用哪些报告工具，不再把整份报告一次性塞给模型。
2. 涉及原话、风险、节奏或改写时，必须先调用对应工具取证。
3. Agent 必须通过 `submit_report_answer` 提交答案、逐字稿 ID、风险项 ID、框架阶段和置信度。
4. Evidence Validator 校验 ID 是否真实且经过工具读取，并核对答案中的原话、时间点及引用/否定语境。
5. 未提交结构化答案、校验失败或 Agent 异常时，直接返回基于报告结构的本地答案，不放行未校验模型文本。
6. 服务端会去掉 Markdown 符号，并将“一定限流/封禁”等过度确定表达收敛为风险提示。
