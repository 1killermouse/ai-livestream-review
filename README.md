...
报告追问单独使用一个 ReAct Agent，按问题自主选择逐字稿检索、风险查询、框架查询、改写建议和 RAG 知识检索工具。这样把自主 Agent 用在适合动态决策的问答环节，同时保持主分析链稳定。
...
3. ReAct 循环失败时回退到原有报告问答；模型仍不可用时，返回基于报告结构的本地答案。
4. 服务端会去掉 Markdown 符号，并将“一定限流/封禁”等过度确定表达收敛为风险提示。
